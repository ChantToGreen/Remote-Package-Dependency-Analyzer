 ...
 * Package operations:
 * -------------------
 * Provides a set of basic graph class for dependecy analysis
 *
 * Required files
 * -------------------
 * CsGraph.cs
 ...
 * Maintenance History:
 * --------------------
 * ver 1.2 : 29 Oct 2018 changes made by Yilin Cui
 * - added Reset() function to clear information stored in the graph
 * - added Contains to the CsNode, to see if contains certain child
 * ver 1.1 : 23 Aug 2018
...
namespace CsGraph
{
    ...
    public class CsNode<V, E>
    {
        ...
        //-------------<Dose it contains certan child?>-----------
        public bool ContainsChild(string childName)
        {
            foreach (CsEdge<V, E> child in children)
            {
                if (child.targetNode.name == childName)
                    return true;
            }
            return false;
        }
    ...
    class CsGraph<V, E>
    {
        ...
        //------------<Reset the graph for new information
        public void Reset()
        {
            startNode = null;
            name = null;
